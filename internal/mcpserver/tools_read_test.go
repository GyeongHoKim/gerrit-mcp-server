...
func TestListChangeMessagesTool(t *testing.T) {
	t.Parallel()
	...
	const body = `[
	  ...
	  {"id": "m2", "date": "2026-07-30 09:05:00.000000000", "_revision_number": 1,
	   "tag": "autogenerated:jenkins", "message": "Build Successful",
	...
	result := callTool(t, srv, "list_change_messages", map[string]any{"change_id": "12345"})

	if result.IsError {
		t.Fatalf("list_change_messages reported an error: %s", resultText(t, result))
	}
	...
	// The automated message is the reason the tool exists; it must be there
	// and marked as automated.
	for _, want := range []string{"Alice Adams", "Uploaded patch set 1.", "Jenkins", "Build Successful", "autogenerated:jenkins"} {
		if !strings.Contains(got, want) {
			t.Errorf("output does not mention %q:\n%s", want, got)
		}
	}
...
func TestReadToolsReportGerritFailures(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"list_change_messages": {
			args:   map[string]any{"change_id": "12345"},
			status: http.StatusNotFound,
			want:   "listing change messages on 12345",
		},
